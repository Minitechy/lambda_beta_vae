...
def visualize_mi_heatmap(mi_matrix, factor_names, save_path, beta, seed, lambda_=None):
    ...
    ax.set_xticklabels([f'$x_{{{i+1}}}$' for i in range(mi_matrix.shape[0])], fontsize=10)
    ax.set_yticklabels(factor_names, fontsize=13)
    ...
    for col, ds_name in enumerate(datasets):
        ...
        for row, (metric, metric_label) in enumerate(zip(metrics, metric_labels)):
            ...
            for i, j in np.ndindex(len(CONFIG['lambdas']), len(CONFIG['betas'])):
                ax.text(j, i, f'{heatmap_data[i, j]:.2f}', ha='center', va='center', color='black', fontsize=15)
            ...
            ax.set_ylim(-0.5, len(CONFIG['lambdas']) - 0.5)
    plt.tight_layout()
